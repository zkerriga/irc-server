...
def oper_invalid_password() -> Test:
    return Test(
        test_name="NICK BASE",
        commands=[
            f"PASS pass",
...
if __name__ == "__main__":
    if os.system(f"make -C {config.BINARY_SERVER_PATH}"):
        exit(1)
    oper_base().exec_and_assert()
    # oper_invalid_password().exec_and_assert()
    # oper_invalid_name().exec_and_assert()